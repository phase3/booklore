...
BookLore uses SQLite as its embedded database, so no separate database container is required.
...
3. (Optional) `podman pull ghcr.io/booklore-app/booklore:latest` to pre-pull the image
  * If you have a slow connection, this is recommended because systemd will time out if the image pull takes too long.
4. Run `systemctl --user daemon-reload` to pick up the new Quadlet unit.
5. Start the pod with `systemctl --user start booklore-pod.service`
...
# Data Persistence
BookLore stores all its data (SQLite database, indexes, images, etc.) in the `/app/data` volume. Make sure to persist this volume to retain your library state across container restarts.
